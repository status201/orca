...
(Use a proxy that returns the token for authenticated users)  

Tokens are generated by the ORCA (System) Administrator

Either by:

### Option 1: Using Tinker (Quick & Easy)
...
Then in Tinker:
```php
// Find or create the user for the external developer
$user = \App\Models\User::where('email', 'developer@example.com')->first();

// Or create a dedicated API user if needed:
// $user = \App\Models\User::create([
//     'name' => 'TinyMCE Integration',
//     'email' => 'tinymce-api@yourcompany.com',
//     'password' => bcrypt('some-secure-password'),
//     'role' => 'editor'
// ]);

// Generate the token (name it something descriptive)
$token = $user->createToken('tinymce-java-integration');

// This is the token to share - it's only shown ONCE!
$token->plainTextToken
```
Copy the output of $token->plainTextToken — this is what you give the developer.

### Option 2: Create a Simple Admin Command

We will create an Artisan Command for this in the future and make a web interface for creating and revoking API tokens. For now, a one-off, Tinker works fine.
...
1. The API Token (from the step above)
...
- Tokens don't expire by default — you can configure expiration in config/sanctum.php if needed
- Revoke a token if compromised:
  // In Tinker - revoke all tokens for a user
  $user->tokens()->delete();

// Or revoke a specific token by ID
$user->tokens()->where('id', 1)->delete();
- View existing tokens for a user:
  $user->tokens

The developer can then use endpoints like GET /api/assets, GET /api/assets/{id}, etc. The RTE_INTEGRATION.md file has detailed examples for TinyMCE
integration they might find useful.
