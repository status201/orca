...
<div class="mb-6 border border-gray-200 rounded-lg overflow-hidden">
    <button @click="showMetadata = !showMetadata" type="button"
        data-testid="batch-metadata-toggle"
        ...
        <span class="flex items-center gap-2">
            {{-- Every field belongs in this chain, or filling only that field leaves the collapsed
                 header reporting nothing is set. Pinned by tests/e2e/upload-metadata.spec.js. --}}
            <template x-if="metadataTags.length > 0 || metadataReferenceTags.length > 0 || metadataLicenseType || metadataCopyright || metadataCopyrightSource || metadataDateObtained">
                <span class="text-xs text-blue-600 font-medium" data-testid="batch-metadata-set">
    ...
    <div x-show="showMetadata" x-collapse x-cloak data-testid="batch-metadata-panel" class="border-t border-gray-200 px-4 py-4 space-y-4 bg-white">
        ...
        <div>
            ...
            <div class="flex space-x-2 relative">
                <div class="flex-1 relative">
                    <input type="text"
                           data-testid="batch-metadata-tag-input"
                    ...
                    <div x-show="metadataShowSuggestions && metadataTagSuggestions.length > 0"
                         x-cloak
                         data-testid="batch-metadata-suggestions"
                        ...
                        <template x-for="(suggestion, index) in metadataTagSuggestions" :key="suggestion.type + '-' + suggestion.id">
                            <div @mousedown.prevent="metadataSelectSuggestion(suggestion)"
                                 data-testid="batch-metadata-suggestion"
                                 :data-tag-name="suggestion.name"
                ...
                <button type="button" @click="metadataAddTag"
                        data-testid="batch-metadata-tag-add"
            ...
            <div class="flex flex-wrap gap-2 mt-2" x-show="metadataTags.length > 0 || metadataReferenceTags.length > 0" x-cloak>
                <template x-for="(tag, index) in metadataTags" :key="'u-' + index">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-700"
                          data-testid="batch-metadata-tag"
                          :data-tag-name="tag">
                        <span x-text="tag"></span>
                        <button type="button" @click="metadataRemoveTag(index)" data-testid="batch-metadata-tag-remove" class="ml-2 hover:text-blue-900">
                ...
                <template x-for="(tag, index) in metadataReferenceTags" :key="'r-' + tag.id">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-orange-100 text-orange-700"
                          data-testid="batch-metadata-ref-tag"
        ...
        <div>
            ...
            <select x-model="metadataLicenseType"
                    data-testid="batch-metadata-license"
        ...
        {{-- Date Obtained --}}
        {{-- No <x-char-counter> and no maxlength: a DATE column has no character width, so there is
             no ColumnLimits cap to surface (input-validation.md REQ-7 is about silent truncation).
             Left blank it is simply not applied — applyUploadMetadata() skips null and ''. --}}
        <div>
            <label for="batch-metadata-date-obtained" class="block text-sm font-medium text-gray-700 mb-2">
                {{ __('Date Obtained') }}
                <span class="text-gray-500 font-normal">{{ __('(optional)') }}</span>
            </label>
            <input type="date" id="batch-metadata-date-obtained" x-model="metadataDateObtained" data-testid="batch-metadata-date-obtained"
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orca-black focus:border-transparent text-sm">
        </div>
